...
(() => {
    (function (lookup, list) {
        ...
        htmx.on(lookup, "htmx:configRequest", function ({ detail, target }) {
        ...
        htmx.on(list, "htmx:load", function ({ target: list }) {
            console.log("htmx:load");
        });

        htmx.on(list, "htmx:beforeSwap", function ({ target: list }) {
            /* cleanup before swapping content */
        });

        htmx.on(list, "htmx:afterSwap", function ({ target: list }) {
            ...
            htmx.findAll(list, ":scope > li").forEach(item => {
                htmx.on(item, "click", function ({ target: el }) {
        ...
        htmx.on(list, "htmx:beforeSettle", function ({ target: list }) {
            console.log("htmx:beforeSettle");
        });

        htmx.on(list, "htmx:afterSettle", function ({ target: list }) {
